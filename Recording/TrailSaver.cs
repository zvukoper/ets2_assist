...
namespace ETS2_Assist_GUI
{
    public partial class MainForm
    {
        ...
        private string GenerateTrailHtml(string compactData, JObject? meta, JObject? mapData)
        {
            ...
            sb.AppendLine("        /* Индикаторы pitch/roll с иконками */");
            ...
            sb.AppendLine("        #prPanel .pr-row { display:flex; gap:12px; align-items:center; margin-bottom:4px; }");
            ...
            sb.AppendLine("    </div>");
            sb.AppendLine("</div>");
            sb.AppendLine("<div id=\"prPanel\"></div>");
            ...
            // ОБЪЯВЛЯЕМ ВСЕ КОНСТАНТЫ, НЕОБХОДИМЫЕ В ПЛЕЕРЕ
            sb.AppendLine("// ================================================================");
            ...
            sb.AppendLine("            // Обновляем расширенную панель с иконками");
            ...
            sb.AppendLine("// ================================================================");
            sb.AppendLine("// ПАНЕЛЬ PITCH/ROLL С ИКОНКАМИ (как на миникарте)");
            sb.AppendLine("// ================================================================");
            ...
            sb.AppendLine("    // Обновляем экстремумы");
            ...
            sb.AppendLine("    const panel = document.getElementById('prPanel');");
            sb.AppendLine("    if (!panel) return;");
            sb.AppendLine("    // Иконки SVG (как на миникарте)");
            sb.AppendLine("    const rollIcon = `<svg width=\"40\" height=\"40\" viewBox=\"0 0 40 40\" style=\"display:inline-block;\"><rect x=\"10\" y=\"10\" width=\"20\" height=\"20\" rx=\"2\" fill=\"none\" stroke=\"#4ecdc4\" stroke-width=\"2\" transform=\"rotate(${-rollDeg}, 20, 20)\" /><line x1=\"20\" y1=\"5\" x2=\"20\" y2=\"35\" stroke=\"#4ecdc4\" stroke-width=\"1\" opacity=\"0.3\" /><line x1=\"5\" y1=\"20\" x2=\"35\" y2=\"20\" stroke=\"#4ecdc4\" stroke-width=\"1\" opacity=\"0.3\" /><circle cx=\"20\" cy=\"20\" r=\"3\" fill=\"#4ecdc4\" /></svg>`;");
            sb.AppendLine("    const pitchIcon = `<svg width=\"40\" height=\"40\" viewBox=\"0 0 40 40\" style=\"display:inline-block;\"><rect x=\"10\" y=\"10\" width=\"20\" height=\"20\" rx=\"2\" fill=\"none\" stroke=\"#ff6b6b\" stroke-width=\"2\" transform=\"rotate(${pitchDeg}, 20, 20)\" /><line x1=\"20\" y1=\"5\" x2=\"20\" y2=\"35\" stroke=\"#ff6b6b\" stroke-width=\"1\" opacity=\"0.3\" /><line x1=\"5\" y1=\"20\" x2=\"35\" y2=\"20\" stroke=\"#ff6b6b\" stroke-width=\"1\" opacity=\"0.3\" /><circle cx=\"20\" cy=\"20\" r=\"3\" fill=\"#ff6b6b\" /></svg>`;");
            sb.AppendLine("    panel.innerHTML = `");
            sb.AppendLine("        <div style=\"display:flex; gap:12px; align-items:center; margin-bottom:4px;\">");
            sb.AppendLine("            <div style=\"display:flex; align-items:center; gap:6px;\">");
            sb.AppendLine("                <span style=\"font-size:11px; color:#ff6b6b;\">Pitch</span>");
            sb.AppendLine("                <strong style=\"color:#ff6b6b; font-size:13px;\">${pitchDeg.toFixed(1)}°</strong>");
            sb.AppendLine("                ${pitchIcon}");
            sb.AppendLine("            </div>");
            sb.AppendLine("            <div style=\"display:flex; align-items:center; gap:6px;\">");
            sb.AppendLine("                <span style=\"font-size:11px; color:#4ecdc4;\">Roll</span>");
            sb.AppendLine("                <strong style=\"color:#4ecdc4; font-size:13px;\">${rollDeg.toFixed(1)}°</strong>");
            sb.AppendLine("                ${rollIcon}");
            sb.AppendLine("            </div>");
            sb.AppendLine("        </div>");
            sb.AppendLine("        <div style=\"display:flex; justify-content:space-between; gap:8px; font-size:10px; color:#6a7b94; margin-top:2px; flex-wrap:wrap;\">");
            sb.AppendLine("            <span>Pitch min: ${pitchRollState.pitchMin.toFixed(1)}°</span>");
            sb.AppendLine("            <span>Pitch max: ${pitchRollState.pitchMax.toFixed(1)}°</span>");
            sb.AppendLine("            <span>Roll min: ${pitchRollState.rollMin.toFixed(1)}°</span>");
            sb.AppendLine("            <span>Roll max: ${pitchRollState.rollMax.toFixed(1)}°</span>");
            sb.AppendLine("        </div>");
            sb.AppendLine("        <div style=\"margin-top:4px; text-align:right;\">");
            sb.AppendLine("            <button id=\"resetPitchRollBtn\" style=\"background:#2a3545; border:1px solid #4a5a6a; color:#d0def0; border-radius:4px; padding:1px 8px; font-size:9px; cursor:pointer; pointer-events:auto;\">Сбросить экстремумы</button>");
            sb.AppendLine("        </div>");
            sb.AppendLine("    `;");

...
- Дублирование `BuildInfo.Version` в footer Map Editor 2 после `ETS2 Assist / Map Editor 2` для контроля кеша.
- «История игрового редактора»: `%LocalAppData%\ETS2_Assist\autosave\game_editor_points.json`, загрузка при старте, автоматическая запись новых точек, клик создаёт новую точку, порядок после «Сохранённые», удаление точки/очистка истории через красные X.
- Системные имена: `MapEditor...` для клика по пустой карте, `GameMapEditor...` для Ctrl+Shift+X из игрового редактора.
- Строгий валидатор обязательных полей: толстая красная рамка, блокировка/красная рамка «Сохранить» и требуемый tooltip.
...
- Статусбар пуст — фикс: при map2-ready принудительное обновление индикатора.
- Частота сбора координат 250мс → 100мс.
- Блокировка основного окна — фикс: ShowDialog → немодальный Show.
- **СЛЕДУЮЩИЙ ШАГ:** верифицировать v39.91 (а) «Редактор запущен» (lime); (б) основное окно
  доступно; (в) Ctrl+Shift+X — мгновенный фокус + точка.
- **ПОСЛЕ ВЕРИФИКАЦИИ — НОВЫЙ БОЛЬШОЙ ЗАПРОС (сессия 13, приоритет):** см. ниже.

# СЕССИЯ 08.09.2026 (21) — v39.90: мониторинг редактора + статусбар + мгновенный фокус
- Bridge: фоновый поток — раз в 5с проверка запущен ли игровой редактор; если запущен — сбор
  координат каждые 250мс в кэш. index.html: «Редактор запущен» (lime) в статусбаре.
  MapEditor2Form: таймер 1с обновления индикатора. Ctrl+Shift+X: мгновенный фокус на наш
  редактор → GetLastPosition (кэш) → точка.
- **СЛЕДУЮЩИЙ ШАГ:** верифицировать v39.90 (а) «Редактор запущен» в статусбаре; (б) Ctrl+Shift+X
  — мгновенный фокус + точка; (в) координаты в map_editor_coords.txt.
- **ПОСЛЕ ВЕРИФИКАЦИИ — НОВЫЙ БОЛЬШОЙ ЗАПРОС (сессия 13, приоритет):** см. ниже.

# СЕССИЯ 08.09.2026 (20) — v39.89: координаты редактора по требованию (Ctrl+Shift+X)
- Убран постоянный опрос координат и статус-бар (текст+кнопка). Всё по требованию.
- Ctrl+Shift+X (при открытом Map Editor 2): ReadCurrentPosition → запись в
  Logs/map_editor_coords.txt → создание точки MapEditor<X><Y><Z>, зум, фокус на «Отображаемое имя».
- **СЛЕДУЮЩИЙ ШАГ:** верифицировать v39.89 (а) статус-бар чистый; (б) Ctrl+Shift+X создаёт точку
  с зумом и фокусом; (в) координаты в map_editor_coords.txt.
- **ПОСЛЕ ВЕРИФИКАЦИИ — НОВЫЙ БОЛЬШОЙ ЗАПРОС (сессия 13, приоритет):** см. ниже.

# СЕССИЯ 08.09.2026 (19) — v39.88: кнопка убрана, координаты слева, Ctrl+Shift+X создаёт точку
- Кнопка «Получить координаты» убрана. Координаты редактора — слева от координат курсора
  через `|`. Ctrl+Shift+X (при открытом Map Editor 2) создаёт точку с координатами из редактора
  (иначе — AR pin).
- **СЛЕДУЮЩИЙ ШАГ:** верифицировать v39.88 (а) кнопки нет; (б) «Редактор:» слева; (в) Ctrl+Shift+X
  создаёт точку из координат редактора.
- **ПОСЛЕ ВЕРИФИКАЦИИ — НОВЫЙ БОЛЬШОЙ ЗАПРОС (сессия 13, приоритет):** см. ниже.

# СЕССИЯ 08.09.2026 (18) — v39.87: фикс deadlock кнопки + позиция «Редактор:»
- **Координаты ПОЯВИЛИСЬ в статус-баре (bridge работает!).** Кнопка вешала (deadlock:
  MessageBox внутри обработчика WebView2) — фикс через BeginInvoke. «Редактор:» был слева
  впритык к «города» — фикс: appendChild вместо insertBefore (теперь справа от координат курсора).
- **СЛЕДУЮЩИЙ ШАГ:** верифицировать v39.87 (а) кнопка без зависания; (б) «Редактор:» справа.
- **ПОСЛЕ ВЕРИФИКАЦИИ — НОВЫЙ БОЛЬШОЙ ЗАПРОС (сессия 13, приоритет):** см. ниже.

# СЕССИЯ 08.09.2026 (17) — v39.86: тестовая кнопка «Получить координаты»
- **Exit работает (v39.85 подтверждён).** Координаты НЕ появились — проблема визуальная.
- **СДЕЛАНО:** кнопка «Получить координаты» в статус-баре Map Editor 2 → OK-диалог с X/Y/Z
  из bridge (GetLastPosition). Bridge хранит _lastPosition.
- **СЛЕДУЮЩИЙ ШАГ:** верифицировать кнопку (диалог с координатами при открытом Map editor).
  Затем чинить визуальное отображение координат (маркер/статус под картой).
- **ПОСЛЕ ВЕРИФИКАЦИИ — НОВЫЙ БОЛЬШОЙ ЗАПРОС (сессия 13, приоритет):** см. ниже.

# СЕССИЯ 08.09.2026 (16) — v39.85: фикс UI-потока CoreWebView2 + Exit-диалог без owner
- **ЗАДАЧА 2 (координаты):** КОРЕНЬ — PushPositionToWebViews трогал webView.CoreWebView2 на
  фоновом потоке (PollLoop) → исключение "CoreWebView2 can only be accessed from the UI thread"
  → координаты не показывались + Fatal Error. ФИКС: проверка CoreWebView2/ExecuteScriptAsync
  перенесены внутрь PushPosition (маршалит на UI-поток через form.BeginInvoke).
- **ЗАДАЧА 1 (Exit):** КОРЕНЬ — DefaultDesktopOnly НЕЛЬЗЯ с owner (недопустимая операция).
  ФИКС: DefaultDesktopOnly БЕЗ owner (поверх всех окон, включая игру); fallback тоже без owner.
- **СЛЕДУЮЩИЙ ШАГ:** верифицировать v39.85 (а) координаты редактора под картой; (б) Exit при
  запущенной игре — диалог виден, выход работает.
- **ПОСЛЕ ВЕРИФИКАЦИИ — НОВЫЙ БОЛЬШОЙ ЗАПРОС (сессия 13, приоритет):** см. ниже.

# СЕССИЯ 08.09.2026 (15) — v39.84: фикс Exit-диалога + координаты редактора
- **ЗАДАЧА 1 (Exit виснет при запущенной игре):** КОРЕНЬ — невидимый MessageBox подтверждения
  выхода (без owner/DefaultDesktopOnly, появлялся за полноэкранной игрой). ФИКС в ConfirmExit:
  owner=this + MessageBoxOptions.DefaultDesktopOnly + fallback + логирование. v39.84.
- **ЗАДАЧА 2 (координаты редактора не под картой):** рассинхрона data НЕТ. КОРЕНЬ — bridge
  регистрировал WebView2 только через Application.Idle, который не срабатывает при модальном
  ShowDialog(MapEditor2Form). ФИКС: RegisterWebView → internal, явный вызов из
  MapEditor2Form.InitializeAsync. v39.84.
- **СЛЕДУЮЩИЙ ШАГ:** верифицировать у пользователя (а) Exit при запущенной игре — диалог виден,
  выход работает; (б) в Map Editor 2 под картой появляются координаты редактора (X/Y/Z) и маркер.
- **ПОСЛЕ ВЕРИФИКАЦИИ — НОВЫЙ БОЛЬШОЙ ЗАПРОС (сессия 13, приоритет):** см. ниже.

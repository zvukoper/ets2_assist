...
- **Инструмент проверки:** `ollama_usage_v2.bat` (headless Edge, выделенный профиль)
  → `SESSION: NN.N | RESET: <время до сброса>` (напр. `45 minutes`).
...
- **ЛОГИКА РЕСУРСА — ПРОЦЕНТЫ ПЕРВИЧНЫ (НОВОЕ ПРАВИЛО, 01.09.2026, задача пользователя):**
  ...
  ```
- **Если лимиты не парсятся / просит авторизацию** (`SESSION: UNKNOWN`): выдать пользователю
  страницу авторизации через `ollama_login.bat` и **ждать авторизации**, затем повторить проверку.

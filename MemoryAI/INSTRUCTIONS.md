...
- **Первый пункт TODO — оценка лимитов/ресурсов + прогноз** в формате:
  ...
  Прогноз считается по РАЗДЕЛ 1 `MemoryAI\ollama_limits_usage.txt` (сумма ресурсозатратности задач
...
- **Учёт расхода:** после задачи записывать в `MemoryAI\ollama_limits_usage.txt`:
...
- **ПЕРЕОЦЕНКА РАЗДЕЛА 1 (01.09.2026):** периодически (раз в несколько задач или по запросу)
  анализировать `MemoryAI\ollama_limits_usage.txt`, сравнивать прогноз vs факт и корректировать
...
- **ПЕРЕОЦЕНКА РАЗДЕЛА 1 (задача пользователя, 01.09.2026):** периодически (раз в несколько
  задач или по запросу) делать тщательный анализ `MemoryAI\ollama_limits_usage.txt` и
...
- **Раз в день (перед началом всех работ):** перечитать `MemoryAI\ollama_limits_usage.txt`, в этом же
...
- **Модели агента (3 сценария, 01.09.2026):** агент фиксирует своё имя (модель) в каждой записи
  лога `MemoryAI\ollama_limits_usage.txt` (РАЗДЕЛ 2). Модели:

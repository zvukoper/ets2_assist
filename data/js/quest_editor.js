...
function markerLabel(marker){
    if(marker==='yellow_exclamation') return '!';
    if(marker==='yellow_question') return '?';
...
function toTarget(p){
    ...
    return {
        id:key,
        uid:String(p.Uid||key),
        GameName:key,
        RealName:questLabel(p),
        name:questLabel(p),
        Category:'Квестовые',
        category:'__custom',
        Description:'Квестовая точка. Характеристики редактируются в редакторе квестов.',
        Enabled:true,
        ShowInAr:Boolean(p.ArVisible),
        ShowOnMap:Boolean(p.MinimapVisible),
        X:Number(p.X)||0,
        Y:Number(p.Y)||0,
        Z:Number(p.Z)||0,
        x:Number(p.X)||0,
        ...
        Icon:'default',
        LabelStroke:1,
        TriggerRadius:Number(p.TriggerRadiusM)||35,
        CooldownMinutes:0,
        Hidden:0,
        DeleteOnComplete:0,
        DialogId:'',
...
function installCombinedTargets(){
    ...
    try{
        const regular=(Array.isArray(baseTargets)?baseTargets:[]).filter(p=>!isQuestTarget(p));
        window.MapEditor2SetTargets(regular.concat(questPointsAsTargets()));
...
function installSetTargetsWrapper(){
    ...
    window.__ets2QuestSetTargetsWrapped=true;
    window.__ets2QuestOriginalSetTargets=original;
...
function selectQuestPoint(key){
    ...
    const timer=setInterval(()=>{
        ...
        if(button){clearInterval(timer);button.click();setTimeout(queueDecorate,40);return}
        if(++tries>20){clearInterval(timer);queueDecorate()}
    },50);
}
...
function wsSend(payload){
    try{
        ...
        ws.onopen=()=>{try{ws.send(JSON.stringify(payload))}catch{}setTimeout(()=>{try{ws.close()}catch{}},300)};
...
function connect(){
    try{
        const ws=new WebSocket('ws://localhost:8085/');
        ws.onmessage=function(ev){try{const d=JSON.parse(ev.data);if(d&&d.command==='quest_state')applyQuestState(d)}catch{}};
...
installSetTargetsWrapper();
window.chrome?.webview?.postMessage('map2-quest-bridge-ready');
connect();
queueDecorate();
})();

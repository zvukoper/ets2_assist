(function () {
    'use strict';
    var ws=null, questState=null, baseByUid=new Map(), minimapOverlay=null, minimapCtx=null;
    var INTERACTIVE_CATS=new Set(['5ka','cafe','small_cafe','letn_cafe','producti','souvenir','souvenir_shop','shop','minimarket','hotel','motel','hostel','service','service_station','mechanic','garage','tire_repairs','shinomontaz','police','police_man','police_car','fuel_l','refuel_zone','restaurant']);
    ...
    /* Слой квестовых указателей на миникарте по умолчанию СКРЫТ.
       Раньше canvas создавался и сразу рисовался при первом quest_state, из-за
       чего при старте на миникарте на мгновение проступали все точки. Теперь
       слой показывается только когда квестовый интерфейс явно активен. */
    function setMinimapOverlayVisible(visible){
        ensureCanvas();
        if(!minimapOverlay)return;
        minimapOverlay.style.transition='opacity 160ms ease';
        minimapOverlay.style.opacity=visible?'1':'0';
    }
    function ensureCanvas(){var c=document.getElementById('minimapContainer');if(!c)return false;if(!minimapOverlay){minimapOverlay=document.createElement('canvas');minimapOverlay.id='questMinimapOverlay';minimapOverlay.style.cssText='position:absolute;inset:0;width:100%;height:100%;pointer-events:none;z-index:75;opacity:0;transition:opacity 160ms ease';c.appendChild(minimapOverlay);minimapCtx=minimapOverlay.getContext('2d');}var d=devicePixelRatio||1,w=c.clientWidth||1,h=c.clientHeight||1,ww=Math.round(w*d),hh=Math.round(h*d);if(minimapOverlay.width!==ww||minimapOverlay.height!==hh){minimapOverlay.width=ww;minimapOverlay.height=hh;}minimapOverlay.style.width=w+'px';minimapOverlay.style.height=h+'px';minimapCtx.setTransform(d,0,0,d,0,0);return true;}
    ...
    function drawMinimapPointers(){if(!ensureCanvas())return;var ctx=minimapCtx,c=document.getElementById('minimapContainer'),w=c.clientWidth||1,h=c.clientHeight||1;ctx.clearRect(0,0,w,h);if(!questState||!Array.isArray(questState.points))return;questState.points.forEach(function(q){if(!q||!q.MinimapVisible||!q.Marker||q.Marker==='none'||!q.ArOffscreenPointer)return;var p=questPointToScreen(q);if(!p)return;var m=18;if(p.x>=m&&p.x<=w-m&&p.y>=m&&p.y<=h-m)return;drawEdgeArrow(ctx,p.x,p.y,p.cx,p.cy,q.Marker);});}
    ...
    window.addEventListener('resize',ensureCanvas);document.addEventListener('DOMContentLoaded',function(){ensureCanvas();connect();requestAnimationFrame(redrawLoop);});
})();
...
- [ ] **ВЕРИФИЦИРОВАТЬ у пользователя (сессия 33, 29.08.2026):** наложение map_overrides на
  миникарту. Переместить город/POI в редакторе → сохранить → миникарта показывает новую позицию;
  новая пользовательская точка видна на миникарте (белая по умолчанию / цвет из редактора);
  скрытая точка пропадает. Конвейер: points_overrides_data по WS (delta поверх статики,
  debounce 400мс); код: MainForm.PointsOverrides.cs + data\js\points_overrides.js.

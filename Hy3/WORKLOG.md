...
## Сессия 20 — Редактор: кулдаун (реальное время) + система overrides + панель редактирования точки (v36)

### Задача (от пользователя, финальная фича фазы «минимальный прототип»)
- Таймеры/кулдауны целей: пока — ОБЫЧНЫЙ кулдаун с точным указанием минут. Перекус
  должен включаться через 2 минуты после отключения при активации триггера. Время —
  РЕАЛЬНОЕ СИСТЕМНОЕ (не игровое), переживает перезапуск. Будущая логика (разовые
  события/таймлимит/инкрементальный кулдаун/случайный диапазон) — заложить комментарием.
- Панель редактирования точки (правая, всегда открыта): выбор точки → загрузка полей;
  клик по пустому месту → режим создания (координаты из клика); грязное поле → жёлтая
  рамка/звёздочка; кнопки «сохранить»/«отменить»; удаление/отмена изменений; статус
  Вкл/Откл; категория (по умолчанию «Пользовательское»); синяя рамка для новых/override;
  серая 3px для отключённых; синяя 3px для точек на кулдауне + суффикс «(кулдаун)».
- Система map_overrides: папка `map_overrides`, `load_order.txt` (сверху — высший
  приоритет; грузим снизу вверх), `custom_map1.json` (создаётся при первом запуске).
  При запуске: статические данные, затем поверх — overrides по load_order. Сохранение
  пишет ВСЮ точку целиком в выбранный файл overrides. Верхняя панель 30px: выпадающий
  список файлов overrides (не из load_order — с `*`), кнопка «Папка», стрелка+число для
  реордера load_order (0 = удалить). CTRL+C копирует выбранную точку как JSON (формат
  overrides). «Только выбранные» — прячет все кроме выделенной.

### Реализовано
- **Кулдаун (реальное время):** `QuestsManager` — `RandomTargetState.CooldownUntil` (UTC);
  `target_zone_enter` игнорирует вход, пока `CooldownUntil > now`; `CompleteTargetById`
  пишет `cooldown_until` (ISO UTC) + `status=inactive`, а не поминутный декремент;
  `DecrementCooldowns` пересчитан на реальное время (раз в минуту проверяет метку,
  сбрасывает `status=active` по истечении, переживает перезапуск). `quest_snack` →
  `cooldown: 2` (data/js/websocket.js). Миникарта (data/js/targets.js) прячет точки на
  кулдауне (`status=inactive` + `cooldownUntil` в будущем).
- **PointData.cs (новый):** единая модель точки со ВСЕМИ свойствами (идентификация,
  координаты, внешний вид, триггер/кулдаун, диалог/награды) + `PointField[]` (метаданные
  полей: Required/ReadOnly/Hidden — устаревшие НЕ удаляются, помечаются OFF/READONLY).
  Метод `Clone()`.
- **map_overrides в редакторе:** `EnsureOverridesInit` (папка+load_order+custom_map1),
  `LoadLoadOrder`/`SaveLoadOrder` (порядок снизу→вверх), `ApplyOverridesToModel`
  (наложение по GameName), `LoadTargets` перестроен на `_pointModel` (Dict<GameName,PointData>).
- **Панель редактирования:** `BuildEditPanel` (верхняя панель overrides + правая панель
  полей по `PointData.Fields` через рефлексию), `SelectPoint`, `EnterCreateMode`,
  `LoadPointIntoPanel`, `OnFieldChanged` (грязные поля → жёлтая рамка выделения),
  `SaveCurrentPoint`/`CommitNewPoint`/`CancelChanges`/`DeleteCurrentPoint` (Удалить /
  ОТМЕНИТЬ ИЗМЕНЕНИЯ), `CopySelectedAsJson` (CTRL+C), `WritePointToOverrideFile`.
  Отрисовка (OnPaint): синяя 3px на кулдауне, серая 3px отключена, синяя 2px override/новая,
  белая/жёлтая 2px выделение; суффикс «(кулдаун)».

### НЕ доделано (дефёрнуто, следующие шаги)
- **Рантайм-наложение overrides на миникарту** (приложение пока не отдаёт map_overrides
  миникарте — overrides работают пока только в редакторе). Нужен механизм отдачи/чтения
  overrides миникартой при запуске.
- Перетаскивание выбранной точки мышью (курсор «кулак»); кнопка «отмена» на КАЖДОЕ поле;
  чекбоксы видимости у каждой категории в левом дереве; «Отключенные» как отдельная
  категория-фильтр; точное поведение стрелки-реордера (сейчас базовое перемещение по индексу).

### Версия / сборка
- `36` EDITOR-OVERRIDES. EXE / build.txt / manifest = **`1.0.38.36-EDITOR-OVERRIDES-08.29-1247`**
  (синхронизированы ПОСЛЕ сборки вручную: csproj не перезаписывает бейджи).

---

## Сессия 21 — Редактор: доделки (v37)

### Задача
Завершить редактор (рантайм-наложение overrides на миникарту — позже). Доделать
отложенные детали панели: перетаскивание точки мышью, per-field «отмена», чекбоксы
видимости категорий в левом дереве, группа «Отключенные».

### Реализовано
- **Перетаскивание точки:** `OnMouseDown` (ЛКМ по точке) → `SelectPoint` + `_dragId`;
  `OnMouseMove` живёт обновляет `X/Z` точки в `_pointModel`, перерисовывает, обновляет
  поля X/Z в панели и помечает их грязными; `OnMouseUp` завершает; курсор `Cursors.SizeAll`
  («кулак»/перемещение). `OnMouseClick` не создаёт точку, если было перетаскивание
  (`_dragMoved`). После перетаскивания точка готова к сохранению («сохранить»).
- **Per-field «отмена»:** в `BuildFieldControls` у каждого редактируемого поля кнопка
  «отмена» (справа); `ResetField(key)` возвращает контрол к значению `_editingCopy`,
  снимает флаг грязи, перерисовывает.
- **Чекбоксы видимости категорий:** `_sidebar.CheckBoxes = true`; родительские узлы
  (Цели/Города/категории POI/Отключенные) несут `Name`=ключ категории и `Checked` из
  `_catVisible`; `_sidebar.AfterCheck` переключает `_catVisible[key]` → `RequestRender`.
  `OnPaint` скрывает точки/города/POI по `_catVisible` (отключённые — группой «Отключенные»).
- **Группа «Отключенные»:** в `PopulateSidebar` отдельный узел со всеми точками
  `Enabled==false` (центрирование по клику работает).

### Версия / сборка
- `37` EDITOR-FINISH. EXE / build.txt / manifest = **`1.0.38.37-EDITOR-FINISH-08.29-1305`**
  (синхронизированы ПОСЛЕ сборки вручную).

---

## Сессия 22 — Правки редактора (10 багов/хотелок пользователя) → 38 EDITOR-FIXES

### Контекст
Пользователь нашёл 10 проблем в правке точек; «наложение в рантайме» отложено. Задачи:
плотный макет без гориз. прокрутки + поле «Описание»; валидатор системного имени
(latin/digits/_)+проверка уникальности+подсветка обязательного; критично — клик по точке
под курсором НЕ создаёт новую точку; выбор по имени в сайдбаре; тултипы на всех кнопках;
тёмно-оранжевый фон у изменённого поля; SAVE реально пишет custom_map1; «Папка» слева;
выбор в сайдбаре центрирует+грузит панель; CTRL+C копирует JSON выделенных; мультивыбор
(Ctrl+Клик) + вписывание всех выделенных в карту.

### Реализовано
- **Макет панели:** ширина 340, `HorizontalScroll.Enabled=false`; `BuildFieldControls`
  плотная сетка (row gap 3, label 8.5, control h22, undo 22×44). Обязательные подписи —
  **жирные** (`FontStyle.Bold`). Новое поле **«Описание»** (`Description`, multiline,
  Height 70/92, 5 строк, ScrollBars.Vertical). Поле `Description` добавлено в `PointData`
  и `PointData.Fields`.
- **Системное имя:** `OnFieldChanged("GameName")` санитизирует ввод (только `[a-z0-9_]`,
  принудительно lowercase) через `_sanitizing` (защита рекурсии). `CheckGameNameUnique()`
  (на `LostFocus`) → красная подпись «Имя уже существует, сделайте имя уникальным».
  `RefreshRequiredHighlight()` краснеет, если режим создания и имя пустое.
- **EnterCreateMode:** `GameName` намеренно пустое (обязательное, красная подсветка),
  `RealName="Новая точка"`, `_selectedIds.Clear()`. Сохранение блокируется, если
  `GameName`/`RealName` пусты или X,Z оба 0 (`ValidatePoint`).
- **КРИТ: курсор по точке не создаёт точку.** `OnMouseClick` переписан: `HitTarget`
  (редактируемая цель) → `SelectPoint`/`ToggleSelect` (Ctrl) и return; городы/POI под
  курсором → копируют координаты и return; только по-настоящему пустое место →
  `EnterCreateMode`. `IsOverAnyMapPoint` больше не нужен отдельно — логика в клике.
- **Сайдбар:** узлы целей/отключенных несут `Name=id` (gameName); `AfterSelect` →
  `SelectPoint`/`ToggleSelect` (Ctrl) + `CenterOn`. Выбор в сайдбаре центрирует карту,
  выделяет на карте (белая рамка) и грузит панель.
- **Мультивыбор:** `_selectedIds` (HashSet); `ToggleSelect` добавляет/убирает; при >1
  выбранных — `FitToSelection` (подгоняет масштаб/центр под все выделенные с паддингом
  1500). `OnPaint` рисует рамку выделения для ВСЕХ `_selectedIds` (жёлтая, если первичная
  и есть грязные поля).
- **Тултипы:** `_editTip` на кнопках действий (Сохранить/Удалить/Отменить/Создать),
  per-field «отмена» («Отменить изменение ТОЛЬКО этого поля...»), комбо overrides
  («Файл оверрайдов (верх=приоритет)»), стрелках перемещения («Поднять/Опустить
  приоритет»), кнопке «Папка» («Открыть папку map_overrides»), чекбоксе «Только
  выбранные».
- **Тёмно-оранжевый фон изменённого поля:** `DirtyBg=Color.FromArgb(120,60,12)`.
  `OnFieldChanged` красит текстовые (не ReadOnly) контролы; `ResetField` и
  `LoadPointIntoPanel` сбрасывают в `Color.FromArgb(40,48,62)`.
- **CTRL+C копирует JSON выделенных:** `this.KeyPreview=true`; `KeyDown` перехватывает
  Ctrl+C → `CopySelectedAsJson()` пишет в буфер **массив** JObject всех `_selectedIds`
  (`SuppressKeyPress` чтобы не копировать текст поля).
- **SAVE реально пишет:** `WritePointToOverrideFile` теперь `Directory.CreateDirectory`,
  логирует абсолютный путь файла. Ранее точка могла не писаться, если `_dirtyFields`
  пуст (drag обновлял `_pointModel`, но кнопка «сохранить» была скрыта) — теперь drag
  помечает X/Z грязными, сохранение видимо и пишет изменённые координаты.
- **«Папка» слева:** перенесена перед подписью «overrides:» (сделано в сессии 21,
  подтверждено/сохранено).

### Версия / сборка
- `38` EDITOR-FIXES. EXE / build.txt / manifest = **`1.0.38.38-EDITOR-FIXES-08.29-1404`**
  (синхронизированы ПОСЛЕ сборки вручную; manifest перезаписан целиком — ранее был
  повреждён ключ `build`).

---

## Сессия 23 — Выделение/чекбоксы/перетаскивание/макет (правки по фидбеку)

### Контекст (фидбек пользователя)
1. Выделялись/грузились только «Цели»; у городов/POI курсор менялся, но левый клик лишь
   копировал координаты. 2. Поля в панели обрезаны сверху, высота не увеличена. 3. Клик по
   точке на карте выделял, но координаты помечались грязными и точка иногда сразу
   смещалась (drag запускался с первого клика). 4. Перерисовка сайдбара при загрузке точки
   (мерцание) — только для «Цели». 5. «Описание» должно быть сразу после «Отображаемое
   имя». 6. Чекбоксы у точек в сайдбаре не работали. 7. Клик по точке на карте должен
   ставить чекбокс в сайдбаре; Ctrl+клик добавляет ещё; Ctrl+C — JSON всех выделенных.

### Реализовано
- **Универсальное выделение ВСЕХ точек.** Ключ выделения: для целей/отключенных —
  `gameName`, для городов — `"city:<name>"`, для POI — `"poi:<uid>"`. Справочник
  `_selectLookup` (`RebuildSelectLookup`) хранит координаты всех точек. `SelectKey`/
  `ToggleKey` заменяют `SelectPoint`/`ToggleSelect` (старые — обёртки). `ApplySelectionToPanel`
  грузит панель ТОЛЬКО для редактируемых (есть в `_pointModel`); для городов/POI панель
  не перезагружается.
- **Левый клик по ЛЮБОЙ точке (цель/город/POI) выделяет её и ставит чекбокс в сайдбаре**
  (`OnMouseClick` через `HitTarget`/`HitCity`/`HitPOI`). Для городов/POI в буфер идут
  координаты (как и раньше), но точка теперь ещё и выделяется.
- **Перетаскивание НАЧИНАЕТСЯ ТОЛЬКО со второго клика** — `OnMouseDown` ставит `_dragId`
  только если точка УЖЕ в `_selectedIds` (и не зажат Ctrl). Первый клик — чистое выделение
  (никаких грязных координат, точка не едет). `SelectKey` не сбрасывает правки при
  повторном клике по уже выбранной точке (чтобы drag не «съедал» dirty).
- **Чекбоксы точек в сайдбаре работают:** `AfterCheck` для листа toggл'ит `_selectedIds`
  и вписывает выделенное в карту (`FitToSelection`). Родительские узлы (категории) по-прежнему
  переключают видимость. Программная установка чекбоксов защищена `_suppressCheck`.
- **Лёгкое обновление выделения в сайдбаре** — `UpdateSidebarSelection` (а не полная
  `PopulateSidebar`), поэтому при выборе/снятии точки нет мерцания перестройки дерева.
  `PopulateSidebar` тоже проставляет `Checked`/`BackColor` по `_selectedIds`.
- **Подсветка выбранных на карте** распространена на города/POI (`_selectedIds.Contains`
  `"city:..."` / `"poi:..."` в `OnPaint`).
- **Макет панели:** увеличены вертикальные отступы (label Top=2, control Top=20, row
  Height 46 / GameName 62 / Description 104), добавлен `Margin(0,0,0,4)` между строками —
  текст/поле больше не обрезаются сверху.
- **«Описание» перенесено сразу после «Отображаемое имя»** (`PointData.Fields`): теперь в
  группе «Основное», индекс 2.
- **CTRL+C** копирует JSON всех `_selectedIds`: для целей — `PointDataToJObject`, для
  городов/POI — объект `{key,name,x,z}`.

### Версия / сборка
- `39` EDITOR-SEL. EXE / build.txt / manifest = **`1.0.38.39-EDITOR-SEL-08.29-1449`**
  (синхронизированы ПОСЛЕ сборки вручную).

---

## Сессия 24 — Delta-сохранение в overrides + перенос только изменённых полей

### Контекст (фидбек пользователя)
- «Изменения точки после нажатия кнопки не сохраняются» (надо перепроверить модель
  сохранения/загрузки).
- В выбранный файл overrides должна писаться **ТОЛЬКО грязная (изменённая) информация**
  (только изменённые свойства точки), а не вся точка целиком.
- После сохранения должна вызываться команда кнопки «Обновить»: данные из файлов
  overrides перезагружаются и применяются на карту. После этого грязные поля УЖЕ не
  грязные и не подсвечиваются оранжевым (загружены из файлов, а не изменены в редакторе).
- Если изменились только координаты — в файл overrides пишется только системное имя и
  координаты. При обновлении точки грузятся из статичных данных, затем поверх накладываются
  overrides **по системному имени**; совпавшая статичная точка заменяется сохранённой из
  overrides (только по тем полям, что есть в overrides — delta-merge).
- Грязными становятся ТОЛЬКО свойства, изменённые пользователем в редакторе.
- Макет панели: убрать лишний воздух между полями (максимально компактно вертикально),
  но поля НЕ должны обрезаться; высота полей +25%.

### Реализовано
- **Delta-запись в overrides** (`WritePointToOverrideFile`): для НОВОЙ точки пишется
  полная `PointDataToJObject`; для существующей — только `_dirtyFields` (через `FieldJson`
  — маппинг ключа поля → JSON-имена, напр. `X/Y/Z → coords`, `Enabled → status+enabled`,
  `RealName → realName`, `Description → description`). Прочие поля в файле overrides
  сохраняются (deep clone существующей записи). Если изменены только координаты — запись
  содержит `gameName` + `coords` (ровно то, что требовал пользователь).
- **Delta-наложение при загрузке** (`ApplyJObjectToPoint` + `ApplyOverridesToModel`):
  выделен общий метод, применяющий к точке ТОЛЬКО присутствующие в JObject поля. Статическая
  точка + override с тем же `gameName` → override-поля замещают статические (остальное
  остаётся из статики). `PointDataFromJObject` теперь тоже через `ApplyJObjectToPoint`.
  Добавлено чтение/запись `description`.
- **Сохранение вызывает «Обновить»** (`SaveCurrentPoint`): после `WritePointToOverrideFile`
  вызывается `LoadTargets()` (перезагрузка статики + overrides, `ApplyOverridesToModel`),
  затем панель перезагружается из ВОССТАНОВЛЕННОЙ модели и `_dirtyFields.Clear()` — оранжевая
  подсветка гаснет, т.к. значения пришли из файлов, а не из правок редактора.
- **Грязные только от редактора**: `_dirtyFields` наполняется исключительно в
  `OnFieldChanged` (и при drag — X/Z). После сохранения/обновления — пуст.
- **Макет**: строки полей теперь `AutoSize=true` (панель сама подгоняет высоту под
  содержимое → поля НЕ обрезаются), межстрочный `Margin=2` (компактно). Высота контролов
  увеличена: текстовые `26` (+~30%), `Описание` `95` (+~25%), кнопка «отмена» `24`.
  Подпись `Top=0`, контрол `Top=15` — плотно, без воздуха.

### Проверка (надо в рантайме)
- Изменить у статической точки только координаты (перетащить) → «сохранить» → в
  `custom_map1.json` должна появиться запись `{gameName, coords}`; после авто-обновления
  точка на карте стоит в новых координатах, оранжевой подсветки нет.
- Изменить `RealName` → запись содержит `gameName, realName` (статические цвет/радиус
  сохраняются из статики при reload).
- Создать новую точку → запись полная.

### Версия / сборка
- `40` EDITOR-DELTA. EXE / build.txt / manifest = **`1.0.38.40-EDITOR-DELTA-08.29-1523`**
  (синхронизированы ПОСЛЕ сборки вручную).

## Сессия 25 (29.08.2026) — ФИКС: overrides сохранялись пустыми
- **КОРЕНЬ БАГА (подтверждён воспроизведением в отдельном repro):** в `WritePointToOverrideFile`
  (MapEditorForm.cs) строка `root["customTargets"] = arr;` ПОСЛЕ `var arr = root["customTargets"] as JArray`
  отвязывала массив от `root` (Newtonsoft-квирк: повторное присваивание токена, уже являющегося
  ребёнком, отвязывает его). Запись `jo` добавлялась в отвязанный `arr`, а `root.ToString()`
  сериализовал ПУСТОЙ массив → файл `custom_map1.json` перезаписывался `{"customTargets":[]}`,
  все сохранённые точки терялись. Симптом: в логе `[SAVE-CONTENT]` пуст, хотя грязных полей 2.
- **Фикс:** назначать `root["customTargets"] = arr` ТОЛЬКО когда массива ещё нет
  (`if (arr == null) { arr = new JArray(); root["customTargets"] = arr; }`). Repro подтвердил:
  после фикса `root["customTargets"].Count == 1`.
- Доп. правки сессии 24 (до фикса): категория «Пользовательское» в сайдбаре (PopulateSidebar),
  точка выделяется после сохранения; расширенное логирование сохранения `[SAVE-*]` и `[INIT]`.
- `41` OVERRIDE-FIX. EXE / build.txt / manifest = **`1.0.38.41-OVERRIDE-FIX-08.29-1615`**.

## Сессия 26 (29.08.2026) — Города/POI загружаются в панель редактирования
- `ApplySelectionToPanel` теперь загружает города и POI в панель (раньше «не редактируются»):
  - Город: системное имя = `id` (→ `GameName`), `RealName` = `name` (или `id`, если пусто),
    Category="Город", координаты из `_cities`.
  - POI: системное имя = `uid` (→ `GameName`), `RealName` = `uid` (отд. имени нет),
    Category = категория оверлея, координаты из `_pois`.
  - Точка регистрируется в `_pointModel` под системным именем (`RegisterEditablePoint`),
    чтобы её можно было сохранить как override.
- `LoadPointIntoPanel`: если `RealName` пустое — в поле подставляется системное имя
  (`GameName`), согласно правилу пользователя.
- `42` CITY-POI-PANEL. EXE / build.txt / manifest = **`1.0.38.42-CITY-POI-PANEL-08.29-1622`**.
- НЕ ПРОВЕРЕНО в рантайме у пользователя.

## Сессия 27 (29.08.2026) — Только грязные поля при сохранении + маркер создания
- **ФИКС перезаписи лишнего:** `WritePointToOverrideFile` больше НЕ сбрасывает весь класс `PointData`
  с дефолтами. Если запись в overrides УЖЕ есть (`existing != null`) — дельта-слияние (сохраняем
  прочие поля, перезаписываем только грязные + `gameName`). Если записи НЕТ (`existing == null`,
  т.е. новая точка ИЛИ первая правка статической) — пишем ТОЛЬКО грязные поля + `gameName`
  (не дублируем исходные/не существующие данные).
- `EnterCreateMode`: для новой точки авто-грязные `X/Y/Z`, `GameName`, `RealName`; прочие — только
  при ручном изменении.
- Карта: в режиме создания (`_createMode`) рисуется временный серый маркер с перекрестьем
  («Новая точка») в точке клика.
- Обязательные поля: `ValidatePoint` требует непустые `GameName`/`RealName` и координаты ≠ 0,0 —
  для новой точки с пустым системным именем сохранение заблокировано (warning).
- `43` DIRTY-SAVE. EXE / build.txt / manifest = **`1.0.38.43-DIRTY-SAVE-08.29-1641`**.
- НЕ ПРОВЕРЕНО в рантайме у пользователя.

## Сессия 28 (29.08.2026) — Защита от пустого/битого файла overrides
- **ПРИЧИНА ошибки:** `custom_map1.json` оказался 0 байт (пустой). `JObject.Parse("")` бросал
  «Error reading JObject from JsonReader. Path '', line 0, position 0» при каждом сохранении.
  `EnsureOverridesInit` пересоздавал файл ТОЛЬКО если он отсутствует, поэтому пустой/битый
  файл оставался и ломал все последующие сохранения.
- Добавлен `LoadOverrideRoot(path)`: пустой/битый/отсутствующий файл трактуется как свежий
  `{"customTargets":[]}` (без исключения). Используется в `WritePointToOverrideFile`,
  `RemovePointFromOverrideFile`, `RemoveAnyOverrideFor`.
- `EnsureOverridesInit` теперь ВОССТАНАВЛИВАЕТ файл, если он пустой (0 байт) или битый
  (нет `customTargets`) — пишет `{"customTargets":[]}`.
- Файл `custom_map1.json` у пользователя восстановлен вручную в `{"customTargets":[]}`
  (потерянные из 0-байтного файла данные невосстановимы).
- `44` OVERRIDE-SAFE. EXE / build.txt / manifest = **`1.0.38.44-OVERRIDE-SAFE-08.29-1650`**.
- НЕ ПРОВЕРЕНО в рантайме у пользователя (ожидается перезапуск и повтор сохранения courier2).
- **НЕ ПРОВЕРЕНО В РАНТАЙМЕ у пользователя:** сохраняется ли теперь точка и появляется ли в
  сайдбаре «Пользовательское» + на карте. (Ожидается подтверждение.)

## Сессия 29 (29.08.2026) — Переименование EXE + правило закрытия перед билдом
- **ПЕРЕИМЕНОВАНИЕ EXE:** старое `ETS2_Assist_GUI.exe` → новое `ETS2_Assist.exe`.
  Реализовано через `<AssemblyName>ETS2_Assist</AssemblyName>` в `ETS2_Assist_GUI.csproj`
  (namespace проекта `ETS2_Assist_GUI` НЕ трогали — он не зависит от имени файла).
  `AppDataPaths.UserDataDirectory` жёстко задан как `ETS2_Assist` (не от AssemblyName) — папка
  данных пользователя не меняется.
- **НОВОЕ ПРАВИЛО (запомнить):** ПЕРЕД `dotnet build` И `dotnet publish` обязательно проверить,
  не запущено ли приложение для тестирования (процесс `ETS2_Assist.exe`, и для совместимости
  `ETS2_Assist_GUI.exe`). Если запущено — **попросить пользователя закрыть приложение** и
  продолжить ТОЛЬКО после подтверждения. Иначе публикация падает на `GenerateBundle`
  (файл залочен) и `publish\data` остаётся неполной.
  Правило вписано в `Hy3/INSTRUCTIONS.md` (раздел «ЛОВУШКА: exe ЗАЛОЧЕН»).
- В `publish\` после билда удалять СТАРЫЙ `ETS2_Assist_GUI.exe`, чтобы не путать пользователя.
- `45` EXE-RENAME. EXE / build.txt / manifest = **`1.0.38.45-EXE-RENAME-...`** (билд после
  закрытия приложения пользователем; см. ниже).
- НЕ ПРОВЕРЕНО в рантайме у пользователя.

## Сессия 30 (29.08.2026) — Города и POI стали полноценными редактируемыми/перетаскиваемыми точками
- **ПРОБЛЕМА (от пользователя):** правка имени POI (id `0x6300514980041`) сохранялась, но на карте
  появлялась НОВАЯ точка в нулевых координатах с тем же id; старая POI не переопределялась. Drag по
  городам/POI не работал — они рисовались/хит-тестались вне единого конвейера `_targets`/`_pointModel`.
- **ПРИЧИНА:** города (`_cities`) и POI (`_pois`) жили в отдельных списках и рисовались/ловились
  отдельно. При наложении overrides `ApplyOverridesToModel` не находил их в `_pointModel` и создавал
  НОВУЮ PointData без координат (=> фантом в 0,0). Drag работал только по `_targets`.
- **РЕШЕНИЕ — унификация городов/POI в единый конвейер:**
  - `PointData`: добавлены runtime-флаги `IsCity`/`IsPoi` (НЕ сериализуются в overrides).
  - `LoadTargets`: города и POI регистрируются в `_pointModel` (ключ = gameName: city id/name, poi uid)
    с `IsCity`/`IsPoi`, реальными координатами и `SourceFile=""`. Overrides теперь накладываются
    ПОВЕРХ них (delta-merge) — координаты сохраняются, фантомов нет.
  - `DrawMap`: убраны отдельные циклы `_pois`/`_cities`; всё рисуется из `_targets` с учётом
    `IsCity`/`IsPoi` (стиль/цвет) и видимости по `pm.Category` (города→«Города», POI→категория).
    Метка POI = `RealName` (по умолчанию = категория «Company», после правки — новое имя).
    Неизвестная категория в `_catVisible` трактуется как видимая (чтобы новые «Пользовательское»/POI
    не пропадали с карты). POI скрываются при `_scale > 10`.
  - `OnMouseClick`/`HoverInfo`/`HitTarget`: города/POI теперь ловятся единым `HitTarget` по `_targets`
    (ключ выделения = gameName). Drag (`OnMouseMove`) обновляет `_pointModel[gameName]` → перетаскиваются.
  - `PopulateSidebar`: города и POI берутся из `_pointModel` (группы «Города» и по категориям POI),
    ключ узла = gameName (совпадает с `_selectedIds`); «Цели» и «Пользовательское» их исключают.
  - `LoadOverlays`: категории POI прописываются в `_catVisible` сразу (до `PopulateSidebar`).
- `46` CITY-POI-EDIT. EXE / build.txt / manifest = **`1.0.38.46-CITY-POI-EDIT-08.29-1713`**.
- НЕ ПРОВЕРЕНО в рантайме у пользователя (ждём подтверждения: правка имени POI→метка меняется,
  старая не дублируется; города/POI перетаскиваются и сохраняют координаты).

## Сессия 31 (29.08.2026) — Диалоги подтверждения + починка исчезновения POI + высота полей
- **БАГ (от пользователя, v46):** из сайдбара исчезла категория «ПОИ» и на карте точек POI нет.
  **ПРИЧИНА:** в конструкторе `LoadTargets()` вызывался ДО `LoadOverlays()`, поэтому `_pois` был
  ещё пуст, и регистрация POI в `_pointModel` не происходила. **ИСПРАВЛЕНО:** порядок в конструкторе
  → `LoadCities(); StartRoadsLoad(); LoadOverlays(); LoadTargets(); PopulateSidebar();`.
  (Точки городов регистрировались нормально, т.к. `_cities` читался в `LoadCities()`, поэтому баг
  касался только POI.)
- **Удаление БЕЗ подтверждения (от пользователя):** было «удаляет без подтверждения». Теперь
  `DeleteCurrentPoint()` показывает `MessageBox` YesNo «Подтверждение удаления» с именем точки;
  удаляет только при `DialogResult.Yes`.
- **Диалог сохранения (от пользователя):** `SaveCurrentPoint()` теперь показывает «Сохранить точку с
  новыми данными?» с JSON-фрагментом (`jo.ToString(Formatting.Indented)`), который БУДЕТ записан в
  overrides; сохраняет только при `DialogResult.Yes`. Для этого вынесен хелпер `ComputeOverrideJObject(pd)`
  (та же дельта-логика, что и в `WritePointToOverrideFile`).
- **Высота полей «срезана сверху» (давняя жалоба):** панель полей строилась с абсолютными `Top=15` и
  фикс-высотой TextBox (26px) — при DPI-скейлинге текст обрезался сверху. **ИСПРАВЛЕНО:** метка теперь
  `AutoSize=true` + `MaximumSize(312,0)`; контрол ставится СТРОГО под меткой (`top = lbl.GetPreferredSize().Height + 3`);
  высота TextBox = `Max(24, TextRenderer.MeasureText().Height + 8)` (DPI-адекватная). Отступы между
  полями компактные (`Margin (0,0,0,3)`). **Обязательные поля** теперь ЖИРНЫЕ + ПОДЧЁРКНУТЫЕ
  (`FontStyle.Bold | FontStyle.Underline`).
- `47` NO-UI-FG-LOG (удалены логи ForceForegroundWindow; см. сессию без номера выше).
- `48` UI-FIXES. EXE / build.txt / manifest = **`1.0.38.48-UI-FIXES-08.29-1736`**.
...
### Версия / сборка
- **`1.0.38.56-LOG-TARGET-FIX-08.30-2109`** — exe = build.txt = manifest = ProductVersion.
  data=873 файла; SyncDataToPublish — 392 файла; изменённые JS в publish идентичны
  исходникам (MD5: targets.js, points_overrides.js, status_bar.js, web_pda_map.html);
  node --check OK.
...
## Сессия 37 (30.08.2026 21:32) — ПАУЗА ПЕРЕД ДИАЛОГОМ + РАЗДЕЛЬНЫЕ ТЕКСТЫ/НАГРАДЫ (57)

### Фидбек пользователя (после v56)
- Точки добавляются, триггеры срабатывают идеально. НО: перед диалогом игра НЕ ВСЕГДА
  ставится на паузу. Требования:
  1) перед диалогом ставим паузу, НО только если игра НЕ на паузе;
  2) после паузы — задержка минимум 2с перед диалогом;
  3) РАЗДЕЛИТЬ текст диалога на контролы: обычный текст отдельно, награды отдельно
     (награды — зелёным жирным);
...
- В логе: `[QUEST] Игра встала на паузу — ждём 2с перед диалогом` (или «УЖЕ на паузе»);
  при сбое — `[QUEST] Пауза не подтвердилась — повторная отправка PAUSE.` и вторая попытка.

---

## Сессия 38 (30.08.2026 22:12) — AR HUD: web_ar_hud.html + кнопка «Запустить AR» (58)

### Задача пользователя
Новый веб-оверлей `web_ar_hud.html`: кнопка «Запустить AR» → страница на ВЕСЬ экран
монитора игры; проецирует ПЕРЕКРЕСТЬЕ на игровой мир — всегда на БЛИЖАЙШУЮ точку 3D;
...
- Кнопка добавлена в Controls. Пред-существующие nullability-варнинги не тронуты.
...
### Что проверить у пользователя (рантайм)
- Кнопка «Запустить AR» → на мониторе игры полноэкранная прозрачная страница;
...
## Сессия 39 (30.08.2026 22:23) — AR ТЕЛЕМЕТРИЯ: WS-ДЕЛЬТА КАК У МИНИКАРТЫ (59)

### Симптом пользователя (v58)
AR-страница показывала «Нет телеметрии фуры».

### ДИАГНОЗ (логи + живые пробы портов)
- **TruckTel жив на :8081** (wsPort=8081 в `%LOCALAPPDATA%\ETS2_Assist\web_data.json`,
найден приложением через Get-NetTCPConnection; **8080 МЁРТВ**).
- **`truck.world.placement` НЕ отдаётся через REST `/api/rest/flat/*`** — проверено:
`/flat/truck` содержит только статические поля (truck.wheel.position и т.п.), БЕЗ placement.
- placement приходит **ТОЛЬКО в WS-дельте** `ws://localhost:{wsPort}/api/ws/delta/flat/?throttle=50`
(тем же каналом, что миникарта: websocket.js → connectWebSocket с wsPort из web_data.json).
- На ПАУЗЕ (`frame.paused=true`) дельта НЕ содержит `truck.*` вовсе — только frame.*;
миникарта хранит ПОСЛЕДНИЙ placement, а мой AR сбрасывал состояние/требовал свежий кадр.

### ФИКСЫ (v59, data/js/ar_hud.js)
1. Собственный WS-канал телеметрии `arConnectTelemetry()`: ws://localhost:{arWsPort}/api/ws/
   delta/flat/?throttle=50, переподключение 1.5с; данные теми же ключами
   (truck.world.placement, truck.head.offset).
2. `readWsPort()` каждые 3с перечитывает web_data.json (его пишет приложение: порт меняется
   8080→8081 и т.п.); при смене порта — переподключение WS.
3. КАЭШ последнего placement: `haveTruck` после первого валидного кадра НЕ сбрасывается;
   на паузе используем последние координаты (поведение миникарты).
4. Удалены: REST-опрос /flat/* (бесполезен для placement) и обёртка applyTelemetryDelta.
5. Статус-строка: если placement не обновлялся >5с («данные Nc назад — пауза?»).

### Версия / сборка
- **`1.0.38.59-AR-WS-FIX-08.30-2223`** — exe=build.txt=manifest; ar_hud.js в publish
  идентичен исходнику (MD5).

### Что проверить у пользователя
- «Запустить AR» при запущенной системе (статический 8082 + TruckTel 8081): статус «AR: N точек»,
  перекрестье указывает на ближайшую точку. При паузе перекрестье остаётся на последнем месте.
- При смене порта TruckTel (перезапуск) AR пересканирует web_data.json сам (до 3с).

---

## Сессия 40 (30.08.2026 22:32) — AR: ПОДБОР БЛИЖАЙШЕЙ ТОЧКИ НА C# (60)
...
- `RefreshArModel()`: копия модели конвейера (LoadStaticCities + LoadStaticPois +
  ...
  isRandom/questType-записи → kind="target" (inactive/кулдун — пропускаются),
